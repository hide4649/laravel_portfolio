function preview_file(id){
	...
	// 結果コードを格納する変数
	var result_code = '';
  var img_id = document.getElementsByTagName('').length;
	...
	for (var i = 0; i < target_element.files.length; i++){
		...
		// コードを更新
		result_code += '<img class="profileimg rounded-circle" src="' + img_url + '" alt="プレビュー">'+'<div class="conteiner">'+'<button type="button" class="btn btn-primary mt-2" id="img" onclick=del(this.id);>削除</button>';
